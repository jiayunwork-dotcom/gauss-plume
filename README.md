...
gauss-plume 是一个高斯烟羽点源扩散核算工具。用户输入源强 Q（g/s）、源描述（直接给有效源高，或给烟囱高度、出口速度、出口内径与烟气/环境温度，由 Briggs 公式计算热抬升）、风速 u、Pasquill 稳定度（A–F）与下风向距离，后端按 Pasquill–Gifford 幂律给出 σy/σz，并用含地面镜像反射项的高斯烟羽公式计算受体浓度。它面向单点源地面/受体浓度与下风向轴线核算，不做天气预报，也不做城市空气质量看板。
